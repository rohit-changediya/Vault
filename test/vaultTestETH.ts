...
import { VaultEthFactory, UsdtFactory, VaultEth, Usdt } from "../typechain";
import { Signer } from "@ethersproject/abstract-signer";
chai.use(solidity);
const { expect } = chai;

describe("VaultETH", async () => {
  let vault: VaultEth;
  let usdt: Usdt;
  let usdc: Usdt;
  let busd: Usdt;
  ...
  it("should deposit USDC tokens", async () => {
    ...
    expect(totalBalanceBefore.add("500000000000000000000")).to.equal(
      totalBalanceAfter
    );
